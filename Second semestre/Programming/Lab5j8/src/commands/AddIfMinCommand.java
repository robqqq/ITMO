...
public class AddIfMinCommand implements Command{
    ...
    public void execute(String[] args) {
        if (args.length == 9){
            try{
                personManager.addPersonIfMin(args[0], Double.parseDouble(args[1]), Long.parseLong(args[2]),
        ...
        if (args.length == 10){
            ...
            try {
                personManager.addPersonIfMin(args[0], Long.parseLong(args[1]), LocalDate.parse(args[2]).atStartOfDay(), commandManager);
            } catch (DateTimeParseException e) {
                logger.log(Level.WARNING, e.toString(), e);
